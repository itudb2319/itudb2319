...
class Database:
    ...
    def initDb(self):
        ...
        from ..Model.authHelper import registerUser
        from werkzeug.security import generate_password_hash
        registerUser('admin', generate_password_hash('123'), role=1)

        tables = ['quiz', 'drivers', 'constructors', 'circuits', 'races', 'qualifying',
    ...
    def insertCSV(self, tableCsvPaths: dict, cursor, conn):
        ...
        for table, csvPath in tableCsvPaths.items():
            csvPaths = join(PATHDIR, 'data', csvPath)
            with open(csvPaths, 'r') as f:
